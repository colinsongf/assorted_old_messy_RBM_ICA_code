...
'''
Original from: http://imonad.com/rbm/restricted-boltzmann-machine/
Modified by: Jason
'''
...
class RBM:
    ...
    def genDataJason(self):
        '''Jasons custom data'''

        mags = uniform(-2, 2, 10)
        offset = uniform(0, 2*pi, 10)
        noiseMag = uniform(.2, 1, 10)

        times = linspace(0, 5, 100)

        data = []
        for tt in times:
            data.append(mags * sin(tt + offset) + normal(0, noiseMag))
